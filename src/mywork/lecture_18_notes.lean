...
∀ ⦃a : α⦄, a ∈ s₁ → a ∈ s₂
--if a satisfys the predicate s1 then a satisyf the predicate s2
-- if s1 a then s2 a
/-
for all objects a of type alpha
if a is in the set s1, then
a is in the subset s2
-/
/-
...
{a | a ∈ s ∧ p a}
/-
set of values that are in s and also satisfy the predicate p
-/
/-
...
def even_primes : set ℕ := sep primes evens
...
def univ : set α :=
λ a, true
--funtion that takes an argument a and returns the proposition true
--lamba lets you just define a function, its called an anonymous function
/-
...
{b | ∃ a, a ∈ s ∧ f a = b}
--if i have a function of value to value i can turn it into a function of set to set
/-
