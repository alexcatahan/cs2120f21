...
- y = log x, viewed as a function from ℝ → ℝ⁺ -- if you restrict domain to domain of definition then it is surjective
- y = x^2, viewed as a function from ℝ → ℝ -- no this isn't
- y = x, viewed as a function from ℝ → ℝ
- y = sin x, viewed as a function from ℝ → ℝ -- no, a conuter example is -2
- y = sin x, as a function from ℝ to [-1,1] ∈ ℝ --yes because the codomain was restricted to equal the range
-/
...
--injective functions are invertable
